<?php
    ...
    $query = "SELECT customerid, address FROM CUSTOMERS WHERE username = '".$_SESSION['user']."'";
    $results = $db->query($query);
    ...
    $query = "SELECT name, stock FROM PRODUCTS WHERE productid IN ('".$item_ids."')";
    $results = $db->query($query);
    ...
    for ($i = 0; $i < $num_results; $i++) {
        $row = $results->fetch_assoc();
        $num_items[$i] = $row['stock'] - $num_items[$i];
        
        //stores names of products bought
        array_push($prod_names, $row['name']);
    }
    
    //creates query to create list of items in order
    $stmt = "INSERT INTO ORDER_CONTAINS VALUES (?, ?, ?, ?, ?)";
    ...
    for ($i = 0; $i < $num_results; $i++) {
        ...
        //tries to insert each item into ORDER_CONTAINS. redirects to homepage if failed
        if (!$query->execute()) {
    ...
    $stmt = "UPDATE PRODUCTS SET stock = ? WHERE productid = ?";
    $query = $db->prepare($stmt);
